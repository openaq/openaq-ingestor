...
def cronhandler(event, context):
    ...
    ascending = (
        settings.FETCH_ASCENDING if "ascending" not in event else event["ascending"]
    )
    pipeline_limit = (
        settings.PIPELINE_LIMIT
        if "pipeline_limit" not in event
        else event["pipeline_limit"]
    )
    realtime_limit = (
        settings.REALTIME_LIMIT
        if "realtime_limit" not in event
        else event["realtime_limit"]
    )
    metadata_limit = (
        settings.METADATA_LIMIT
        if "metadata_limit" not in event
        else event["metadata_limit"]
    )
    ...
    logger.info(
        f"{metadata_limit}/{metadata} metadata, {realtime_limit}/{realtime} openaq, {pipeline_limit}/{pipeline} pipeline records pending"
    )
    ...
    try:
        if realtime > 0 and realtime_limit > 0:
            cnt = 0
            loaded = 1
            while loaded > 0 and cnt < realtime and (time() - start_time) < timeout:
